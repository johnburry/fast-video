-- Migration: Filter search results to exclude low-quality transcripts (v3 - Fixed)
-- This ensures that videos with only music/applause/filler content don't appear in search results

-- 1. Drop ALL existing versions of the semantic search function
-- We need to drop each signature explicitly because there are multiple versions
DROP FUNCTION IF EXISTS search_transcripts_semantic(vector(1536), text, float, int);
DROP FUNCTION IF EXISTS search_transcripts_semantic(vector(1536), text, uuid, uuid, float, int);
...
-- 3. Update the materialized view definition (without rebuilding it yet)
